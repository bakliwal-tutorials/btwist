    ...
    try {
      ...
      if (!firebase.apps.length) {
        const firebaseConfig = {
          ...
          databaseURL:
            "https://btwist-default-rtdb.asia-southeast1.firebasedatabase.app",
          projectId: "btwist",
      ...
      console.error(
        "Firebase initialization failed for password manager:",
        error,
      );
      ...
      this.validPasswordHashes = new Set();
      throw new Error(
        "Authentication system requires database connection. Please refresh and try again.",
      );
    }
  }
  ...
  async markPasswordAsUsed(passwordHash) {
    try {
      if (this.firebaseInitialized) {
        ...
        const usedPasswordRef = this.database.ref(
          `auth/used-passwords/${passwordHash}`,
        );
        ...
        const result = await usedPasswordRef.transaction((currentData) => {
          if (currentData === null) {
            ...
            if (currentData.deviceFingerprint === deviceFingerprint) {
              ...
              console.log(
                "Different device trying to use already used password",
              );
      ...
      if (this.validPasswordHashes.has(passwordHash)) {
        ...
        if (this.usedPasswords.has(passwordHash)) {
          errorDiv.textContent =
            "This password has already been used by another user. Please try a different one.";
          ...
          if (marked) {
            ...
            errorDiv.textContent =
              "This password has already been used on a different device. Please try a different password.";
            errorDiv.style.display = "block";
            document.getElementById("authPassword").value = "";
            document.getElementById("authPassword").focus();
          }
  ...
  async loadConfigFromFirebase() {
    try {
      ...
      if (config) {
        ...
        this.showStatus(
          "EmailJS configuration not found. Please set up configuration.",
          "error",
        );
      }
    } catch (error) {
