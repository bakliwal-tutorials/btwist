...
class PasswordManager {
  constructor() {
    ...
    if (localStorage.getItem('btwist_authenticated') === 'true') {
    ...
    this.validPasswordHashes = new Set();
    this.usedPasswords = new Set();
    
    // Initialize Firebase first, then show auth overlay
    this.initializeFirebase().then(() => {
      this.showAuthOverlay();
    }).catch((error) => {
      this.showErrorOverlay('Database connection failed. Please refresh and try again.');
    });
  }
  ...
  async initializeFirebase() {
    try {
      ...
      // Load/create valid passwords and used passwords
      await this.loadValidPasswords();
      await this.loadUsedPasswords();
      
      console.log('Firebase initialized for password management');
    } catch (error) {
      console.error("Firebase initialization failed for password manager:", error);
      // Block auth until Firebase is available - single-use requires central storage
      this.firebaseInitialized = false;
      this.validPasswordHashes = new Set();
      throw new Error('Authentication system requires database connection. Please refresh and try again.');
    }
  }

  // Load valid passwords from Firebase (or create them)
  async loadValidPasswords() {
    try {
      const validPasswordsRef = this.database.ref('auth/valid-passwords');
      
      // Always use fixed passwords - clear any old cached passwords
      console.log('Setting up fixed password authentication...');
      const passwords = this.getFixedPasswords();
      ...
      // Replace any existing passwords with our fixed set
      await validPasswordsRef.set(passwordHashes);
      this.validPasswordHashes = new Set(Object.keys(passwordHashes));
      
      console.log('Fixed password set loaded with 50 predefined passwords from passwords.md');
    } catch (error) {
      console.error("Failed to load valid passwords from Firebase:", error);
      
      // Fallback to local passwords if Firebase fails
      const passwords = this.getFixedPasswords();
      ...
      this.validPasswordHashes = localHashes;
      console.log('Using local fixed passwords as fallback');
    }
  }

  // Load used passwords from Firebase
  async loadUsedPasswords() {
    try {
      const usedPasswordsRef = this.database.ref('auth/used-passwords');
      
      // Load existing used passwords from Firebase
      
      const snapshot = await usedPasswordsRef.once('value');
      const usedData = snapshot.val();
      
      if (usedData) {
        this.usedPasswords = new Set(Object.keys(usedData));
        console.log(`Loaded ${this.usedPasswords.size} used password hashes from Firebase`);
      } else {
        console.log("No used passwords found in Firebase - all 50 fixed passwords are available");
      }
    } catch (error) {
      console.error("Failed to load used passwords from Firebase:", error);
    }
  }
  ...
  generateDeviceFingerprint() {
    ...
    const fingerprint = {
      ...
      canvas: canvas.toDataURL().substring(0, 200), // Canvas fingerprint (limited)
  ...
  // Mark password as used in Firebase using transaction with device tracking
  async markPasswordAsUsed(passwordHash) {
    try {
      if (this.firebaseInitialized) {
        const deviceFingerprint = this.generateDeviceFingerprint();
        const usedPasswordRef = this.database.ref(`auth/used-passwords/${passwordHash}`);
        
        // Use transaction to ensure atomicity
        const result = await usedPasswordRef.transaction((currentData) => {
          if (currentData === null) {
            // Password not used yet, mark it as used with device info
            return {
              usedAt: Date.now(),
              timestamp: new Date().toISOString(),
            ...
            // Password already used - check if it's the same device
            if (currentData.deviceFingerprint === deviceFingerprint) {
              console.log('Same device trying to reuse password - allowing');
              return currentData; // Keep existing data, allow same device
            } else {
              console.log('Different device trying to use already used password');
              return undefined; // Abort transaction - different device
            }
          }
        });
        
        if (result.committed) {
          this.usedPasswords.add(passwordHash);
          console.log('Password marked as used in Firebase for this device');
          return true;
        } else {
          console.log('Password was already used by a different device');
          return false;
        }
      }
      return false;
    } catch (error) {
      console.error("Failed to mark password as used:", error);
      return false;
    }
  }

  getFixedPasswords() {
    // Fixed 50 passwords from passwords.md
    const passwords = [
      'j;5aB@@Pq&[}-(P4',
      '8B{p3)Y(Na%-,s*,',
      'U5MgND_I8-MYu<lA',
    ...
    console.log('Using fixed password set with 50 predefined passwords');
    return passwords;
  }
  ...
  showAuthOverlay() {
    ...
    // Never show real passwords in UI for security
    ...
    overlay.innerHTML = `
      <div style="position: fixed; top: 0; left: 0; width: 100%; height: 100%; 
        ...
        <div style="background: white; padding: 30px; border-radius: 10px; 
          ...
          <h2 style="text-align: center; margin-bottom: 20px; color: #333;">🔐 Access Required</h2>
          <p style="text-align: center; margin-bottom: 20px; color: #666;">
            Enter the access password to unlock BTwist
          </p>
          ...
          <div id="authError" style="color: red; text-align: center; margin-top: 10px; display: none;">
            Invalid password. Please try again.
          </div>
          <button id="getPasswordBtn" 
                  style="width: 100%; padding: 10px; background: #34a853; color: white; 
          ...
          <div style="margin-top: 15px; font-size: 12px; color: #888; text-align: center;">
            <strong>Note:</strong> Each password can only be used once!<br>
            <em>Contact administrator for valid passwords</em>
          </div>
        </div>
      </div>
    `;
    ...
    // Add proper event listeners (not onclick)
    const unlockBtn = document.getElementById('unlockBtn');
    unlockBtn.addEventListener('click', () => {
      this.authenticate();
    });
    
    // Add Get Password button functionality
    const getPasswordBtn = document.getElementById('getPasswordBtn');
    getPasswordBtn.addEventListener('click', () => {
      window.open('https://t.me/itz_atharva07', '_blank');
    });
    ...
    overlay.innerHTML = `
      <div style="position: fixed; top: 0; left: 0; width: 100%; height: 100%; 
        ...
        <div style="background: white; padding: 30px; border-radius: 10px; 
          ...
          <h2 style="text-align: center; margin-bottom: 20px; color: #d32f2f;">⚠️ Connection Error</h2>
          <p style="text-align: center; margin-bottom: 20px; color: #666;">
            ${message}
          </p>
  ...
  async authenticate() {
    ...
    try {
      ...
      const passwordHash = await this.hashPassword(enteredPassword);
      
      console.log('Authenticating password...');
      console.log('Total valid passwords loaded:', this.validPasswordHashes.size);
      console.log('Total used passwords:', this.usedPasswords.size);
      console.log('Entered password length:', enteredPassword.length);
      
      // Check if password hash is valid
      if (this.validPasswordHashes.has(passwordHash)) {
        // Check if password has already been used
        if (this.usedPasswords.has(passwordHash)) {
          errorDiv.textContent = 'This password has already been used by another user. Please try a different one.';
          errorDiv.style.display = 'block';
          document.getElementById('authPassword').value = '';
          document.getElementById('authPassword').focus();
        } else {
          // Password is valid and unused (or same device) - try to mark it as used
          const marked = await this.markPasswordAsUsed(passwordHash);
          
          if (marked) {
            this.isAuthenticated = true;
            // Store authentication in localStorage so it only asks once per browser session
            localStorage.setItem('btwist_authenticated', 'true');
            document.getElementById('authOverlay').remove();
            // Initialize the main app
            window.emailEditor = new EmailTemplateEditor();
            console.log('Authentication successful - App unlocked');
          } else {
            errorDiv.textContent = 'This password has already been used on a different device. Please try a different password.';
            errorDiv.style.display = 'block';
            document.getElementById('authPassword').value = '';
            document.getElementById('authPassword').focus();
          }
    ...
    unlockBtn.textContent = 'Unlock App';
  }
}
...
class EmailTemplateEditor {
  ...
  updatePreview() {
    ...
    const emailBody = `
      ...
      <p>Please find below the detailed result of <strong>${btestNumber}</strong> that was conducted in the offline mode on <strong>${formattedDate}</strong>.</p>
  ...
  initPWAInstallPrompt() {
    ...
    // You can uncomment and customize this if you want an install button
    /*
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  // Add click outside modal to close (only if emailEditor exists)
  document.getElementById("configModal").addEventListener("click", (e) => {
    if (e.target.id === "configModal" && window.emailEditor) {
